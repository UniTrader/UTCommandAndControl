...
function menu.buttonComm()
	C.RemoveHoloMap(menu.holomap)
	menu.holomap = 0
	local entities = Helper.getSuitableControlEntities(menu.selectedcomponent)
	if #entities == 1 then
		Helper.closeMenuForSubConversation(menu, false, "default", entities[1], menu.selectedcomponent, "facecopilot")
	else
...
function menu.buttonSelectspace()
	C.RemoveHoloMap(menu.holomap)
	menu.holomap = 0
	table.remove(menu.history)
	Helper.closeMenuForSection(menu, false, menu.modeparam[1], { 0, 0, menu.selectedcomponent })
...
function menu.hotkey(action)
	...
	if rowdata ~= "back" then
		...
		elseif action == "INPUT_ACTION_ADDON_DETAILMONITOR_C" then
			if rowdata and (rowdata[1] == "station" or rowdata[1] == "ship") and (not menu.mode) and IsComponentOperational(menu.selectedcomponent) and not IsSameComponent(menu.selectedcomponent, menu.playership) then
				menu.buttonComm()
			end
		...
		elseif action == "INPUT_ACTION_ADDON_DETAILMONITOR_T" then
			...
			if rowdata and (rowdata[1] == "ship") and (not menu.mode) and (not IsSameComponent(menu.selectedcomponent, menu.playership)) and (not menu.onplatform) then
				C.SetSofttarget(ConvertIDTo64Bit(menu.selectedcomponent))
			end
...
function menu.createChildList(isfirsttime)
						...
						if isseqextended then
							if seqidx == 1 and sequence[0] then
								for k, module in ipairs(sequence[0]) do
									...
									}, {"module", module.component, seqidx}, {1, 3})
								end
							end
	...
	-- print(tostring(menu.settoprow) .. ", " .. tostring((not menu.settoprow) and ((menu.setrow and menu.setrow > (16 + (ffi.string(ownerDetails.factionIcon) ~= "" and 3 or 2))) and menu.setrow - 13 or (ffi.string(ownerDetails.factionIcon) ~= "" and 4 or 3)) or menu.settoprow) .. ", " .. tostring(menu.setrow) .. ", " .. tostring(lines))
	local selectdesc = setup:createCustomWidthTable({(menu.componenttype == "galaxy" or menu.componenttype == "cluster" or menu.componenttype == "sector") and (2 * Helper.standardTextHeight) or Helper.standardButtonWidth, 0, Helper.standardButtonWidth, Helper.standardButtonWidth}, false, false, true, menu.taborderoverride and 2 or 1, ffi.string(ownerDetails.factionIcon) ~= "" and 3 or 2, Helper.standardSizeY - offsety + 5, 0, Helper.standardSizeY - offsety, true, (not menu.settoprow) and ((menu.setrow and menu.setrow > (16 + (ffi.string(ownerDetails.factionIcon) ~= "" and 3 or 2))) and ((menu.componenttype == "zone") and (menu.setrow - 13) or (menu.setrow - 7)) or (ffi.string(ownerDetails.factionIcon) ~= "" and 4 or 3)) or menu.settoprow, menu.setrow)
	menu.setrow = nil
	menu.settoprow = nil
	menu.setcol = nil
	...
	setup:addSimpleRow({ 
		...
		"",
		Helper.createButton(Helper.createButtonText(menu.parenttitle, "center", Helper.standardFont, Helper.standardFontSize, 255, 255, 255, 100), nil, false, menu.componenttype ~= "galaxy" and ((menu.mode ~= "selectplayerobject") or (menu.modeparam[4] == 0)), 0, 0, 150, 25, nil, Helper.createButtonHotkey("INPUT_STATE_DETAILMONITOR_BACK", true)),
		"",
		Helper.createButton(Helper.createButtonText(menu.componenttype == "sector" and (IsSameComponent(GetActiveGuidanceMissionComponent(), menu.selectedcomponent) and ReadText(1001, 1110) or ReadText(1001, 1109)) or ReadText(1001, 3216), "center", Helper.standardFont, Helper.standardFontSize, 255, 255, 255, 100), nil, false, menu.componenttype == "sector", 0, 0, 150, 25, nil, Helper.createButtonHotkey("INPUT_STATE_DETAILMONITOR_Y", true)),
		"",
	...
	-- set button scripts
	local nooflines = (ffi.string(ownerDetails.factionIcon) ~= "" and 4 or 3) + (displayedyields and #yields + 2 or 0) + #gates + #jumpbeacons
...
function menu.onRowChanged(row, rowdata)
	--print("onRowChanged: " .. row .. ", row name: " .. tostring(rowdata))
	if menu.holomap == 0 then
		return
	end
			...
			if rowdata[1] == "sequence" then
				...
				if menu.componenttype == "zone" then
					...
					if rowdata[1] == "module" then
						...
						-- DETAILS
						Helper.removeButtonScripts(menu, menu.buttontable, 2, 9)
						SetCellContent(menu.buttontable, Helper.createButton(Helper.createButtonText((menu.mode == "selectplayerobject" or menu.mode == "selectobject") and ReadText(1001, 3102) or ReadText(1001, 2961), "center", Helper.standardFont, Helper.standardFontSize, 255, 255, 255, 100), nil, false, ((not menu.mode) or (menu.mode == "selectobject")) and IsInfoUnlockedForPlayer(menu.selectedcomponent, "name") and (CanViewLiveData(station) or GetComponentData(menu.selectedcomponent, "tradesubscription") or menu.mode == "selectobject"), 0, 0, 150, 25, nil, Helper.createButtonHotkey("INPUT_STATE_DETAILMONITOR_X", true)), 2, 9)
						Helper.setButtonScript(menu, nil, menu.buttontable, 2, 9, menu.buttonDetails)
					elseif rowdata[1] == "gate" or rowdata[1] == "jumpbeacon" then
						...
						-- COMM
						Helper.removeButtonScripts(menu, menu.buttontable, 2, 7)
						SetCellContent(menu.buttontable, Helper.createButton(Helper.createButtonText(ReadText(1001, 3216), "center", Helper.standardFont, Helper.standardFontSize, 255, 255, 255, 100), nil, false, (not menu.mode) and IsComponentOperational(menu.selectedcomponent) and not IsSameComponent(menu.selectedcomponent, menu.playership) and GetComponentData(menu.selectedcomponent, "caninitiatecomm"), 0, 0, 150, 25, nil, Helper.createButtonHotkey("INPUT_STATE_DETAILMONITOR_Y", true)), 2, 7)
						Helper.setButtonScript(menu, nil, menu.buttontable, 2, 7, menu.buttonComm)
						-- DETAILS
						Helper.removeButtonScripts(menu, menu.buttontable, 2, 9)
				...
				elseif menu.componenttype == "container" then
					-- DETAILS
					Helper.removeButtonScripts(menu, menu.buttontable, 2, 9)
					SetCellContent(menu.buttontable, Helper.createButton(Helper.createButtonText(ReadText(1001, 2961), "center", Helper.standardFont, Helper.standardFontSize, 255, 255, 255, 100), nil, false, (not menu.mode) and IsInfoUnlockedForPlayer(menu.selectedcomponent, "name") and CanViewLiveData(menu.selectedcomponent), 0, 0, 150, 25, nil, Helper.createButtonHotkey("INPUT_STATE_DETAILMONITOR_X", true)), 2, 9)
					Helper.setButtonScript(menu, nil, menu.buttontable, 2, 9, menu.buttonDetails)
				elseif menu.componenttype == "sector" then
					...
					-- DETAILS
					Helper.removeButtonScripts(menu, menu.buttontable, 2, 9)
					SetCellContent(menu.buttontable, Helper.createButton(Helper.createButtonText(menu.mode == "selectzone" and ReadText(1001, 3102) or menu.childtitle, "center", Helper.standardFont, Helper.standardFontSize, 255, 255, 255, 100), nil, false, ((menu.mode ~= "selectplayerobject") or (menu.modeparam[4] == 0)) and not menu.mode == "selectspace", 0, 0, 150, 25, nil, Helper.createButtonHotkey("INPUT_STATE_DETAILMONITOR_X", true)), 2, 9)
				...
				elseif menu.componenttype == "galaxy" then
					...
					-- DETAILS
					Helper.removeButtonScripts(menu, menu.buttontable, 2, 9)
					SetCellContent(menu.buttontable, Helper.createButton(Helper.createButtonText(menu.childtitle, "center", Helper.standardFont, Helper.standardFontSize, 255, 255, 255, 100), nil, false, ((menu.mode ~= "selectplayerobject") or (menu.modeparam[4] == 0)), 0, 0, 150, 25, nil, Helper.createButtonHotkey("INPUT_STATE_DETAILMONITOR_X", true)), 2, 9)
	...
	if (not menu.lock) or (menu.lock + 0.5 > GetCurRealTime()) then
		...
		if pickedcomponent ~= 0 then
			...
			elseif menu.searchtext == "" or menu.filterComponentByText(ConvertStringTo64Bit(tostring(pickedcomponent)), menu.searchtext, true) then
				if C.IsComponentOperational(pickedcomponent) and pickedcomponentclass ~= "ship_xs" and pickedcomponentclass ~= "highwayentrygate" and pickedcomponentclass ~= "tradeofferdock" and (menu.componenttype == "zone" or (pickedcomponentclass ~= "gate" and pickedcomponentclass ~= "station" and (not ispickedcomponentship))) and not menu.createChildListRunning then
					...
					menu.createChildList(true)
				elseif pickedcomponentclass == "gate" then
					if menu.componenttype == "sector" then
						menu.lastchild = C.GetContextByClass(pickedcomponent, "zone")
					elseif menu.componenttype == "cluster" then
						menu.lastchild = C.GetContextByClass(pickedcomponent, "sector")
					end
					menu.commanderlist = {}
					menu.lastchildstationdata = {}
					menu.createChildList(true)
...
function menu.onRenderTargetDoubleClick()
	local pickedcomponent = C.GetPickedMapComponent(menu.holomap)
	if pickedcomponent ~= 0 then
		if menu.searchtext == "" or menu.filterComponentByText(ConvertStringTo64Bit(tostring(pickedcomponent)), menu.searchtext, true) then
			local pickedcomponentclass = ffi.string(C.GetComponentClass(pickedcomponent))
			if C.IsComponentOperational(pickedcomponent) and pickedcomponentclass ~= "ship_xs" and pickedcomponentclass ~= "highwayentrygate" and pickedcomponentclass ~= "gate" and (menu.componenttype == "zone" or (pickedcomponentclass ~= "station" and pickedcomponentclass ~= "ship")) and not menu.createChildListRunning then
				local type
				if pickedcomponentclass == "ship" or pickedcomponentclass == "station" then
					type = "ship"
				else
					type = "child"
				end
	...
	if not menu.createChildListRunning then
		if menu.componenttype == "zone" then
			...
			if rowdata then
				...
				elseif rowdata[1] == "module" then
					component = C.GetContextByClass(convertedRowData2, "container")
				end
			end
			if component ~= 0 then
				for i, entry in ipairs(menu.extendedcontainer) do
...
function menu.autoZoomIn(component)
	local pickedcomponent = C.GetMapComponentBelowCamera(menu.holomap)
	if pickedcomponent ~= 0 then
		local pickedcomponentclass = ffi.string(C.GetComponentClass(pickedcomponent))
		if C.IsComponentOperational(pickedcomponent) and ((pickedcomponentclass == "station" and menu.componenttype == "zone") or pickedcomponentclass == "cluster" or pickedcomponentclass == "sector" or pickedcomponentclass == "zone") and not menu.createChildListRunning then
			local type
